<script lang="ts">
	...
	import { validInteger, validURLError } from '$lib/validation.js';
...
{:else}
	...
	>
		...
		<Subheader icon={SettingsLabel}>Setup</Subheader>
		...
		<ol>
			...
			<li>
				Include this venue's <Link to={`/venue/${venue.id}/submissions/new`}>payment link</Link> in author
				instructions and submission confirmations, prompting authors to pay after submission. If you want
				the manuscript ID to be populated automatically and your reviewing platform supports it, you can
				use the URL
				<code>https://reciprocal.reviews/venue/{venue.id}/submission/new?id=[ID]</code>, but replace
			...
			<li>
				...
				<code>[id]</code> with the variable your system uses for manuscript ID. In the email, prompt
				them to add the reviewer to the submission (if they haven't already), evaluate the review,
				and if it meets your venue's review quality requirements, press the
		...
		<Subheader id="settings" icon={SettingsLabel}>Settings</Subheader>
