...
- [x] When a `Submission` is created, the venue's editor is assigned to it — but only when there is exactly one accepted, active volunteer in the venue's top-priority role, and only when that person is not an author of the paper. Anything else assigns nobody: with several editors the choice would be arbitrary, and assigning all of them would bill the venue an editor's compensation per editor per paper, since marking a submission done pays every approved priority-0 assignment on it. The same rule applies per row on bulk import, which may additionally **name the person to seat on each row** in a role the importing admin chooses — venues are not shaped alike, and one venue's per-paper editor column names its associate editors while another's names the holders of its top role. A named person must already be an accepted, active volunteer in the chosen role: seating is not a way to hand out a role, least of all the priority-0 one. A name the venue cannot resolve to exactly one such person **blocks the import** rather than being guessed at, and the database refuses an ineligible one outright, rolling the whole batch back — a half-seated import is harder to reason about than none. At most one priority-0 assignment is ever made per row: the sole-editor rule above still applies to rows that named nobody for that role, and stands down for rows that did. Whoever is left waiting is told (see Notifications), and a submission nobody is editing is flagged on the venue's submissions list until someone takes it.
