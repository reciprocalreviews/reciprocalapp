...
create function isSteward () returns boolean language sql security definer
set
	search_path to '' as $$
...
create policy "Scholar can insert themselves" on public.scholars for insert to anon,
authenticated
with
	check (true);
...
create policy "Scholars can remove themselves" on public.scholars for delete to anon,
authenticated using (
	id=(
		select
			auth.uid ()
...
create function public.handle_new_scholar () returns trigger security definer
set
	search_path to '' language plpgsql as $$
begin
