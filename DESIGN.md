...
- [x] _`scholar`_: For non-invite only roles, volunteer to review for the venue in a particular role. When they first volunteer, a number of tokens specified by for venue `welcome_amount` should be minted and given to the scholar, welcoming them to the community. The grant **settles immediately** rather than waiting on a minter's approval: the amount is standing venue policy, granted at most once per scholar, so per-grant approval adds no oversight the `welcome_amount` setting doesn't already provide — while the delay landed precisely on the newcomer who volunteered in order to afford a submission.
