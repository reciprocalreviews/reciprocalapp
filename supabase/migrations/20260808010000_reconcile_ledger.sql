...
begin
	...
	--
	-- Only stewards, and only those with a verified contact address — the same rule
	-- queue_email enforces, since an unverified address was never confirmed to
...
--------------------------------------
-- Daily, just after the reminder job so the two never contend.
select
	cron.schedule(
		'reconcile-ledger',
		'15 22 * * *',
