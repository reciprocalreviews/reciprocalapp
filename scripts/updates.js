...
function parseChangelog(changelog) {
	...
	lines.forEach((line) => {
		...
		const typeofChangeMatch = line.match(/^### (Added|Fixed|Changed|Removed)$/);
		if (typeofChangeMatch) {
			...
			const update = line.substring(2).trim();
			currentUpdate.changes[currentType.toLowerCase()].push(update);
		}
	});
...
fs.writeFileSync(datePath, JSON.stringify({ date: updates[0].date }, null, 2), 'utf-8');
