...
After changing `Locale.ts`, run `npm run locale` to regenerate `src/lib/locales/LocaleText.json` and validate `en.json`. `LocaleText.json` is a generated artifact that is also checked in — commit the regenerated copy alongside any `Locale.ts` change so the repo stays in sync. CI runs `npm run locale` (regenerate + validate), so a stale committed schema won't break the build, but it will show up as a dirty diff for the next contributor.
