...
2. **Consumer — `resend` function.** [supabase/functions/resend/](supabase/functions/resend/) posts to the Resend API, wrapping the body in the shared branded HTML shell ([supabase/functions/\_shared/emailShell.ts](supabase/functions/_shared/emailShell.ts)) and sending both an HTML and a text/plain alternative. In local dev (when `PUBLIC_SUPABASE_URL` points at 127.0.0.1) it logs to the console instead. **It requires a `service_role` caller** ([\_shared/auth.ts](supabase/functions/_shared/auth.ts)): `verify_jwt` alone accepts any valid project JWT including the public anon key, so without the role check the function was callable by anyone who read the browser bundle. `send_email()` and the `remind-daily` cron therefore authenticate with the `service_role_key` vault secret rather than `anon_key`. The `remind` function carries the same guard.
