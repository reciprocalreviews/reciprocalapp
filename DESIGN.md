...
- Sam gets the URL of the volunteer page and sends an email through various social media platforms, inviting people to review. The page lists every role's volunteers by default; Sam can later restrict a role to the people who have completed work at the venue, or to none of them, without hiding how many there are
...
- [x] RR mirrors a narrow, review-relevant slice of each scholar's **public** ORCID record — affiliation, keywords, recent works, education, outbound links — so that editors can see who a candidate reviewer is without leaving the platform. The mirror is world-readable (it is already public at orcid.org), is written only by RR itself, and is editable by **nobody**, including the scholar it describes. It is derived data: it can always be rebuilt by re-reading ORCID. See the Scholar route below for what is deliberately left out of it.
...
- [x] A role can be set to list its volunteers to everyone, to only those who have completed work at the venue, or to no one outside the people who staff it. A public roster is a credential, and listing someone who has only ever signed up hands them one they have not earned: roles can be collected across venues and displayed without anything having been contributed. Two kinds of role are listed either way, because both are status nobody can award themselves — an invite-only role, where the invitation is the vetting, and the venue's top role, whose holders are the venue's public face. The venue's admins, its editors, and whoever approves assignments to a role always see that role's whole roster, since an approver who cannot see the pool cannot pick anyone out of it; and the **number** of volunteers stays public whichever tier is chosen, so a venue that would rather not name its reviewers still keeps the recruiting signal that says how many it has
...
- [x] A `Scholar` can **erase their account**. Their name, email address, and ORCID iD are permanently removed and they can no longer sign in. The mirrored copy of their ORCID record is deleted outright rather than anonymised — alone among RR's tables it is derived, so there is no reason to keep a husk of it — and nulling their iD is what stops anything fetching it back.
...
- [x] Bids on submissions can be approved by roles that are set to be approving roles for another role (e.g., Associate Editors can approve bids from Reviewers) — but **on the submissions they are seated on**, not across the venue. Holding a role that approves another role is a statement about what kind of work someone does, not a claim on every paper that work happens to touch: the Associate Editor of one paper has no business in the reviewing of the next. So the approver of a submission is whoever holds the approving role **on that submission**, alongside its editor and the venue's admins. Venue admins and a venue's editors are deliberately broader, each for a stated reason — admins run the venue, and editors need to see submissions waiting for an editor in order to claim them.
- [x] A submission's assignments are visible only to the assigned scholar, whoever may approve that assignment on that submission (a venue admin, the submission's editor, or the holder of the approving role on it), and — when a venue runs open (non-anonymous) review — the submission's authors. Scholars can declare conflicts on submissions; a declared conflict always hides that submission's assignments from the conflicted scholar, including in open review.
- [x] The same rule decides which of a venue's submissions a scholar finds at all. Being an accepted volunteer in an approving role does not by itself surface the submissions that role is assigned to; without a seat on the paper, a conflict declared on it, or one of the venue-wide routes above (an author, a volunteer on a biddable role, one of the venue's editors, an admin), it is simply not theirs to see.
...
- [x] ([#27](https://github.com/reciprocalreviews/reciprocalapp/issues/27)): Because ORCID does not provide an email, prompt a newly signed-in scholar to add a contact email and verify ownership via a link (valid **24 hours**) before RR will send them any notifications. Until an email is verified, a persistent banner warns that no notifications will be sent, and RR sends nothing to an unverified address except the verification email itself. The same verification flow is used to change an email later. The link is delivered **only by email** and is never shown in the interface — a verification the requester could read on screen would confirm nothing about who controls the address. Re-visiting a link within its window is safe and still reports success, so a mail scanner or link preview cannot consume it before the scholar clicks.

  A day rather than a quarter-hour because the clock starts when RR sends, not when the message becomes readable, and the gap between those is not ours to control: a mail server that greylists routinely adds ten minutes before the first delivery attempt, and reading a notification on a phone and getting back to a computer can take longer than that. A window that can lapse before the link is reachable is not a security property, it is a dead end. What the link can do is narrow — it sets a contact address, nothing else — so the cost of a longer one is small.

  **A pending verification is visible until it is resolved** ([#164](https://github.com/reciprocalreviews/reciprocalapp/issues/164)), not only on the screen that started it. The profile says which address is waiting, when the link stops working, and offers to send it again; so does the expired-link page, which used to be able to say only "request a new one from your profile" and send the scholar off to retype an address they had already typed. Asking again is always available and never limited, but the button says when it will work rather than refusing after the fact. And when the message never actually left the building, RR says so — waiting on mail that was never sent is the one failure a scholar cannot diagnose from the outside, and being told to check their spam folder for it is worse than useless.
...
- [x] Link to the scholar's ORCID profile (`scholars.orcid`), to help visitors get more information about them. Shown as the full `https://orcid.org/…` URI behind the green iD icon, which is what ORCID's display guidelines ask for.
- [x] Mirror a **narrow, review-relevant slice** of the scholar's public ORCID record: current affiliation (role, department, organization), self-declared keywords, the five most recent works with a count and year span over all of them, most recent education, and outbound links. Anything the record itself lists twice — a paper it holds in two places, an identifier entered by two sources — is shown once, while the count still spans the whole record: the reader is looking at a person's career, not at ORCID's bookkeeping. This reverses an earlier decision to mirror nothing and link out for everything, and the reason is the assigner: an editor choosing a reviewer was opening orcid.org in another tab to answer "who is this person and do they work on this?", and RR knew the iD all along. The slice is **scoped rather than open-ended** — RR is not becoming a profile host, and everything outside it still lives at the other end of the link.
- [x] RR deliberately mirrors **no email** (RR verifies its own contact addresses, [#27](https://github.com/reciprocalreviews/reciprocalapp/issues/27), and must never adopt one it did not verify), **no address**, **no biography**, and **no peer-review records** — the last because publishers populate them so sparsely that an almost-empty section reads to an editor as "this person does not review", which is usually false and discourages exactly what RR exists to encourage. Fundings, memberships, distinctions, services, qualifications, invited positions and research resources are all available and all omitted: none of them bear on whether someone should review a particular paper.
- [x] The mirror is **not editable by anyone**, including the scholar it describes. Its whole value is that it says what ORCID says; a writable copy would be a claim RR was vouching for. A scholar controls it through ORCID's own visibility settings, which is also the consent mechanism — the public API returns only what they marked public, so there is no RR-side toggle to explain.
- [x] **Absence renders as absence.** A scholar RR has not read, or whose record is public but empty, gets no heading, no placeholder and no blanks — a section reading "Affiliation: —" tells a visitor something false about a person. The one asymmetry is failure: a scholar is told when RR could not read their record, because they are the only person who can act on it, and a visitor is not, because on a public page the same words read as an accusation and may only mean ORCID was down when RR asked.
- [x] A `steward` can see how the mirror is doing — how many profiles have been read, how many never have, how many are failing — and refresh it from the About page, which works through the oldest records a batch at a time and reports how many it took. If ORCID has been limiting how fast RR may read, the page says so, because the remedy (registering an API client) is something only a steward can do — ORCID limits how fast it may be read, so this is re-runnable rather than a one-time import.
- [x] Every mirrored section says **where it came from and when it was last read**. That line is load-bearing rather than decorative: it is what keeps the section from reading as Reciprocal Reviews' own assertion about a person rather than as a copy of what they published themselves.
- [x] RR still gathers and displays the expertise data that is specific to peer review (e.g. per-role expertise statements provided when the scholar volunteers). That remains the **primary** signal wherever the two appear together, because it is about reviewing for _this venue_ and the ORCID record is not.
...
- [x] _`editor`_: Export the list of reviewers as a CSV file for use on other plaforms, including ORCID, name, email, expertise, ORCID keywords, role, papers cap, and active status. The mirrored ORCID keywords are their **own column**, never appended to the expertise column: merging them would export a claim the venue never made.
- [x] _`editor`_: Find someone to assign in the venue's list of volunteers. The list is searchable by name, email, and expertise, and can also be narrowed to **expertise keywords**, offered as a row of selectable tags above the list. The keywords are derived from what volunteers actually wrote rather than from a controlled vocabulary: a venue's expertise is its own, and asking every venue to define a taxonomy before its editors can filter would mean no venue could filter on the day it opened. Each tag says how many volunteers claim it and they are offered most-claimed first, counted over whatever the search box has already narrowed to — so the counts answer "what is in front of me now" rather than "what exists somewhere" — but not over the tags already picked, because a list that narrowed to your own selection could never have a second tag added to it. Selecting several shows everyone claiming **any** of them: an editor picking two keywords for a paper that touches both is widening the net, not asking for the few people who claim both. Grouping is case-insensitive, and a keyword is shown in the spelling the most volunteers used. Each role's section says how many volunteers it is showing, and within it those still volunteering come first, then everyone by family name — someone who has stopped is a permanent record of the venue but is not who an editor is scanning for, so they go last rather than interleaved. A volunteer's mirrored **ORCID keywords are shown beneath their expertise, each half of the cell marked by whose claim it is** — the platform's own logo on what the volunteer told this venue, the ORCID iD mark on what their public record says, so either can stand alone without being mistaken for the other and a side with nothing in it simply does not render. They **never become chips or enter the chip counts**: expertise is what this scholar wrote for _this venue about reviewing_, and a chip asserting that a career keyword means "will review this here" would be making the venue's claim on ORCID's behalf. There is a volume argument too — venue expertise runs to a handful of terms where ORCID keyword lists routinely pass twenty, so merging them would push the review-relevant chips off the list entirely. They are **searchable** instead: the search box matches them, because typing a term asks who is associated with it rather than asserting anything.
...
- [x] _`editor`_: Enable or disable bidding **per role** (`roles.biddable`), determining whether its volunteers can bid on the venue's submissions, and set how many approved assignments close bidding on a submission for that role (`roles.desired_assignments`). It is a role-level setting rather than a venue-level one because a venue's roles are not alike: a venue may want its reviewers bidding and its meta-reviewers assigned.
- [x] _`editor`_: Ask a biddable role's volunteers to **bid**, in a short note of the editor's own words. Bidding only works if volunteers actually come and bid, and every other notice RR sends fires on something having _already_ happened — a bid arriving, a role changing, a payout. Nothing said "we are short of bids this week and would like you to look", so the only recourse was to export the volunteer list and mail it from outside RR, which loses the opt-out, the mail log, the delivery tracking and the data download in one step. It is deliberately **not automated**: the useful version of this message is a person asking their community for help, and a scheduled one would be a nag nobody wrote. Whoever may ask is whoever runs the venue — its admins and the holders of its priority-0 role, the same union that vets thank-you notes — and the message is offered only on roles that can actually be bid on, since a nudge to bid sent to people the submissions page shows no bid buttons to is a message nobody can act on. The form says how many people will receive it **before** it is sent, and that number is the number that goes out: someone who has paused, has verified no contact address, or has silenced the notice is excluded from both the count and the send. The note is the only prose anybody writes into RR's mail, and it stays bounded — one paragraph, and any URL in it renders as plain text rather than a link, because branded mail from the platform must not carry a link the platform did not build. There is deliberately **no rate limit**: an editor decides when their community needs asking. What the form does instead is say when the venue last asked and who asked, so two co-chairs do not unknowingly send the same nudge an hour apart — information in service of that judgment rather than a brake on it.
...
- [x] _`editor`_: When assigning, see each candidate's mirrored ORCID affiliation and work count beneath their name, and their ORCID keywords beneath the venue's own expertise statement, each marked with the logo of whoever is claiming it. The assignment field itself resolves an email or iD to a named scholar **before** the assignment is made, and shows that same context — it used to accept the text silently and report a bad address only after the button was pressed. None of this can block assigning: a reviewer must still be assignable when ORCID is unreachable, when their record is private, and when RR has simply never read it.
...
- [x] A bulk import sends one message rather than one per row. Two hundred imported manuscripts are one piece of news, not two hundred. That one message therefore **names neither the reader's role nor the reason they were seated** ([#181](https://github.com/reciprocalreviews/reciprocalapp/issues/181)). It goes to everyone the import seated, in whatever roles the file named them, and by either of two paths — named in the file, or the sole-editor rule above standing in — so it can claim neither. It used to call every recipient the venue's editor and explain that they had been assigned because they were its only one, which was false for anyone holding some other role and for anyone the file had named by name. This is the same argument as the new-volunteer notice's "names the venue's own word for its top role, because that is data and not a fixed title", reaching the opposite conclusion from it: a notice about one person's one role can say what that role is, and a digest covering several of both cannot. It counts **submissions rather than seats** for the same reason — one row can seat one person twice, and a reader told two papers had arrived would go looking for the second.
- [x] The same holds for what the **interface** says back. An action that emails many people confirms it once, naming somebody and counting the rest — "Rigor Russ and 306 others were emailed …" — rather than posting a separate confirmation per recipient. Three hundred of those filled the page, each needing its own dismissal, and buried the page's own content and navigation underneath them. The count is not decoration: a scholar with no verified contact address or one who has silenced the notice is skipped, so who was emailed is never quite who was asked, and the number is what makes that difference visible now that the names are not all listed. A handful still appear individually, because naming two or three people is more useful than counting them.
...
- [x] When a venue's editor **asks a role's volunteers to bid**, tell them. This is the one notice a person composes rather than an event triggering, and it is the counterpart of the bid notice above: that one tells a venue somebody answered, and this one is the venue asking. It is **N private copies, never one shared thread** — the opposite of the new-volunteer notice, and for a stated reason: its readers are the reviewers of a venue, who have no business learning each other's addresses, whereas a welcome is meant to converge into one conversation. Its `Reply-To` is the **sender's own** address, so answering it reaches the editor who wrote it rather than the stewards; an editor who has verified no contact address cannot send one at all, because a personal letter nobody can answer is worse than none. The message names the venue's own word for the sender's job ("Editor", "Area Chair", "Associate Editor") and the role the reader volunteers for, so it is clear both who is asking and why they were asked. On by default: it is infrequent, it comes from a venue the scholar chose to volunteer for, and it is about the very work they volunteered to do — the notices that ship off are the ones too frequent to impose, and a venue asking for bids a few times a cycle is not one of them.
