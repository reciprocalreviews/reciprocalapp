...
- **A volunteer record can no longer be deleted or moved to another venue.** Stopping volunteering has always kept the record and marked it inactive — that is what makes a venue's welcome tokens a one-time thing. But the database still permitted a volunteer record to be deleted outright, or reassigned to a role at a different venue, by anyone willing to call the API directly rather than use the site. Either was a way to be welcomed by the same venue twice. Nothing in Reciprocal Reviews offered those actions; they are now refused outright.
...
- **Every role a venue created was silently a top-priority role.** Priority is not just the order roles are listed in: the venue's first role is the one whose holders act as editors, able to approve any assignment on a submission, edit its author list, and mark it done. New roles were created without a priority and so all landed at the top, giving their volunteers that authority until an admin happened to use the reorder arrows. New roles are now added at the bottom, existing venues with tied roles have been renumbered, and the note explaining the top role no longer appears on several roles at once — nor does it still claim that volunteers for it are assigned to new submissions automatically, which was never true of it.
- Saving an editable field no longer flashes the old value. The field now shows what you typed the moment you save it, with the button showing that a save is in flight, and reopens with your text intact if the save fails. Previously it reverted to the previous value while the save was in progress and switched back a second later, which on a slow connection read as the edit having been lost.
- Validation warnings no longer linger on a field that has been cleared. A field that empties after you submit — the new role name, a new steward, a new minter — used to keep showing the "must not be empty" warning it earned while you were filling it in. Fields now stay quiet until you type in them, and go quiet again when something clears them.
