...
**Two reads that look like one question are two.** The scholar profile's Tasks table is `scholar_tasks()`, and the approver rows below it come from `scholar_approver_roles()` ([20260920000000](supabase/migrations/20260920000000_editor_tasks_when_actionable.sql)). They were one: the load fetched every open assignment, rendered that array as the task list, and mapped the _same_ array to role ids to find what the scholar approves on. Narrowing what the table displayed therefore silently deleted the approver's "pending assignment" and "compensation to approve" rows — a coupling nothing in either call site announced. They are separate functions now, and `scholar_approver_roles` deliberately keeps the _old_, wider filter so that narrowing the task list cannot reach it. Both answer for `auth.uid()` rather than taking a scholar id, because the table is drawn only on your own profile; the profile load gates them on the signed-in user, which it reads from the root layout's `claims.sub` and **not** from `scholar`, a key this route's own `+layout.ts` shadows with the _viewed_ scholar's row.
...
`scholar_tasks` reuses that same blocker definition — approved, uncompleted, priority > 0 — as its test for whether a submission is ready for its editor to close, so the profile's task list and this button cannot disagree about which papers can be finished.
...
`seq` is a `bigint` from a sequence, and it is what makes the order of history well-defined. `created_at` defaults to `now()`, which is transaction _start_ time, so every row a single RPC writes carries an identical timestamp — `create_submission` inserts one charge per author that way. Sorting on `created_at` alone therefore leaves ties the planner may break differently per query, and a `LIMIT`/`OFFSET` over an unstable sort can return one row on two pages while skipping another; the three paginated transaction lists all end their sort `created_at desc, seq desc` for this reason. They **lead** it with `status asc`, which puts proposed transactions on the first page whatever their date — ordering by date alone buried a proposal nobody had answered behind however many pages of settled history had accumulated since, with nothing on screen to say it was there. Ascending is proposed → approved → declined because PostgreSQL orders an enum by the position its labels were _declared_ in, not alphabetically, and `transaction_status` was created as `('proposed', 'approved', 'canceled')` with the third later renamed; nothing in the TypeScript can see that, so [supabase/tests/invariants/transaction_status_order.sql](supabase/tests/invariants/transaction_status_order.sql) asserts it — a label added ahead of `proposed` would silently invert every transaction list on the platform. Note that a sequence gives _insertion_ order rather than _commit_ order, so anything treating `seq` as a replication watermark should compare against `pg_snapshot_xmin(pg_current_snapshot())` instead of assuming `max(seq)` is final.
...
**A priority-0 assignment is a standing seat, not a piece of work,** and that distinction needs its own rule. `create_submission` and `bulk_import_submissions` seat a venue's sole editor on every submission it receives, and the row stays approved and uncompleted until `mark_submission_done` pays it — so "holds an open assignment" is true of an editor for every paper the venue has ever handled. The profile's task list read exactly that and told the editor their whole venue was a personal to-do list. **`scholar_tasks()`** ([20260920000000](supabase/migrations/20260920000000_editor_tasks_when_actionable.sql)) asks the narrower question instead: a priority-0 row earns a place only when the submission is waiting on the editor — nobody seated in a non-editor role yet, or every seated one settled.

It is `SECURITY DEFINER`, and here that is load-bearing rather than habitual, for the same reason as `submission_has_editor`. Answering the question means aggregating over the submission's **sibling** assignment rows, and the `assignments` SELECT policy admits those to an editor only `and not public.isConflicted(submission)`. Run as the caller, a conflicted editor would see no siblings, every one of their submissions would report "nobody is seated", and the rows would go straight back onto the list — the failure points in the direction that adds rows rather than hides them. [scholar_tasks.sql](supabase/tests/rpc/scholar_tasks.sql) pins that case, and it fails if the function is ever "simplified" to `SECURITY INVOKER`.
...
Two further signals are **advisory** and deliberately do not flip `ok`: `unattributed_mints` and `orphan_proposals`. Both are real signal in production and should be zero there, but [supabase/seed.sql](supabase/seed.sql) inserts tokens directly, so every development and CI database carries `unattributed_mints` permanently. `orphan_proposals` is the softer case: the seeded proposal now ships with supporters, so a freshly reset database reports zero — but [proposalLifecycle.end.ts](end2end/proposalLifecycle.end.ts) creates proposals the suite never supports, so any database a Playwright run has touched still has some. Advisory is still the right treatment for both. Folding them into `ok` would make the check red everywhere and therefore ignored — which is the failure mode a monitoring check is most prone to.
...
- **Shared seed, shared helpers.** All specs share the one Supabase DB seeded by [supabase/seed.sql](supabase/seed.sql); use the `SEED` constants and `sql()` helper from [end2end/test-utils.ts](end2end/test-utils.ts) rather than re-declaring UUIDs or `psql` wrappers. This covers the seed's **user-visible text** as much as its ids: a spec that types a scholar's name or a submission title into a filter has quietly made the seed's prose part of the contract, and then the local database cannot be made more legible without a red suite to wade through first. No scholar name, submission title, manuscript ID, venue title, currency name or mirrored ORCID string appears as a literal in `end2end/` — each comes from `SEED`, so rewriting the seed's copy is a one-file edit. Where a spec depends on a fragment's _matching behaviour_ rather than its spelling (one that picks out exactly one scholar, one that is deliberately ambiguous), `SEED.filters` names it for the behaviour and documents the property, so a rename is re-checked rather than blindly pasted. Restore any shared row you mutate (usually in a `finally`). [end2end/global-setup.ts](end2end/global-setup.ts) resets the DB before each local run, so no manual `npm run reset` is needed.
